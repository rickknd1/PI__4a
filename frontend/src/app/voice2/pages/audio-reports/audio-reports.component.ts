...
export class Voice2AudioReportsComponent implements OnInit {
  ...
  loadReports(): void {
    this.loading = true;
    const url = this.statusFilter === 'ALL' ? this.api : `${this.api}?status=${this.statusFilter}`;
  ...
  openDecisionModal(report: AudioReport): void {
  ...
  submitDecision(): void {
    ...
    const types = [this.decisionTypeWarning ? 'WARNING' : null, this.decisionTypeDeleteAudio ? 'DELETE_AUDIO' : null]
      .filter(Boolean)
      .join(',');

    this.decisionSubmitting = true;
    this.http
      .patch(`${this.api}/${this.decisionReport.id}/status`, {
        status: 'REVIEWED',
        decisionType: types,
  ...
  updateStatus(report: AudioReport, status: string): void {
  ...
  deleteReport(report: AudioReport): void {
