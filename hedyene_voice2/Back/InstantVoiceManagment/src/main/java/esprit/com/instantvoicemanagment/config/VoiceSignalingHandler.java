    ...
    private void forwardToTarget(String channelId, String targetUserId,
        ...
        if (target != null && target.isOpen()) {
            try {
                ...
                // Target disconnected between isOpen() check and send.
        ...
        for (Map.Entry<String, WebSocketSession> entry : sessions.entrySet()) {
            if (!entry.getKey().equals(excludeUserId) && entry.getValue().isOpen()) {
                try {
                    ...
                    // Session closed concurrently: drop stale session to avoid noisy logs.
                    Map<String, WebSocketSession> channelSessions = channelMap.get(channelId);
                    if (channelSessions != null) {
                        channelSessions.remove(entry.getKey());
